 ...
 *
	...
	I may suffice to just have each thread just create it's own socket when needed.

	for now it will just, create sockets with the proper protocol, and provide methods for
...
int set_dest_addr(struct sockaddr_in* addr, char* addr_string,int port){
	...
	int ret = inet_pton(AF_INET,addr_string,&(addr->sin_addr));
	return ret;
}
